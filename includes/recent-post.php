 ...
 * @package makplus
 */
if( !class_exists('makplus_Recent_Post') ){
	class makplus_Recent_Post extends WP_Widget{
		/**
		...
		function __construct(){
			...
			$widget_options = array(
				'description'					=> esc_html__('makplus recent post here', 'makplus'), 
				'customize_selective_refresh' 	=> true,
			);

			parent:: __construct('makplus_Recent_Post', esc_html__( 'Recent Post : makplus', 'makplus'), $widget_options );
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts','makplus' );
                ...
                <?php while($posts->have_posts()) : $posts->the_post();  ?>
	               	<li>
	               		<div class="widget-recent-post-thumb">
							<a href="<?php the_permalink() ?>"><?php the_post_thumbnail( 'makplus-77x64' ); ?></a>
	               		</div>
				    ...
				    <?php while($posts->have_posts()) : $posts->the_post();  ?>
				        <li>
				            <div class="rc-post-thumb">
				                <a href="<?php the_permalink() ?>"><?php the_post_thumbnail( 'makplus-100x75' ); ?></a>
				            </div>
...
function makplus_Recent_Post(){
	register_widget('makplus_Recent_Post');
}
add_action('widgets_init','makplus_Recent_Post');